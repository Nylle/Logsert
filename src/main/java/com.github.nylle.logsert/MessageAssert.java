...
public class MessageAssert extends AbstractAssert<MessageAssert, List<ILoggingEvent>> {
    ...
    public static MessageAssert assertThat(List<ILoggingEvent> actual) {
    ...
    public MessageAssert withLevel(Level level) {
        ...
        this.level = level;
        this.candidates = this.candidates.stream().filter(x -> x.getLevel().equals(level)).collect(toList());
    ...
    public MessageAssert withMdcEntry(String key, String value) {
        ...
        this.candidates = this.candidates.stream().filter(x -> x.getMDCPropertyMap().containsKey(key) && x.getMDCPropertyMap().get(key).equals(value)).collect(toList());
    ...
    public MessageAssert withException(Class<? extends Throwable> throwableClass) {
        ...
        this.candidates = this.candidates.stream().filter(x -> x.getThrowableProxy().getClassName().equals(throwableClass.getName())).collect(toList());
    ...
    public MessageAssert withException(Class<? extends Throwable> throwableClass, String message) {
        ...
        this.candidates = this.candidates.stream().filter(x -> x.getThrowableProxy().getClassName().equals(throwableClass.getName()) && x.getThrowableProxy().getMessage().equals(message)).collect(toList());
    ...
    public MessageAssert withException(Throwable throwable) {
        ...
        this.candidates = this.candidates.stream().filter(x -> reflectThrowable(x.getThrowableProxy()).equals(throwable)).collect(toList());
